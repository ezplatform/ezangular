...
function getScopesFromManifest(manifest) {
  if (!isImportMap()) {
    throw new Error(`Invalid function call, Scopes is not supported for Sofe implementations.`);
  }
    ...
    lock.writeLock(releaseLock => {
      ...
      const resultPromise = ioOperations
        ...
        .then(data => {
          ...
          return ioOperations.writeManifest(newImportMapString, env).then(() => {
            releaseLock();
            return json;
          });
        })
...
exports.modifyService = function (env, serviceName, url, remove, packageDirLevel = 1) {
  return modifyLock(env, json => {
    ...
    if (remove) {
      ...
      if (
        ...
        const pathToPackageDir = packageDirLevel === 1 ? './' : '../'.repeat(packageDirLevel - 1);
        const address =
          url.indexOf('http://') !== -1 || url.indexOf('https://') !== -1
            ? new URL(pathToPackageDir, url).href
            : new URL(pathToPackageDir, `https://example.com${url.startsWith('/') ? url : '' + url}`).pathname;
