...
const express = require('express'),
  ...
  { verifyValidUrl, findUrlsToValidateInScopes, findUrlsToValidateInServices } = require('./verify-valid-url.js'),
  { verifyInputFormatForServices, verifyInputFormatForScopes } = require('./verify-valid-input-format'),
...
app.use(
  morgan(
    function (tokens, req, res) {
      return [tokens.method(req, res), tokens.url(req, res), tokens.status(req, res), JSON.stringify(req.body)].join(' ');
    },
    {
      skip: (req, res) => {
        return (req.url === '/health' || req.url === '/') && res.statusCode == 200;
      }
    }
  )
);
...
function sendError(res, ex, prefix) {
  const status = ex && ex.message && /No such environment/.test(ex.message) ? 404 : 500;
...
// });
// app.get("/mf/", healthEndpoint);
app.get('/mf/environments', function (req, res) {
  res.send({
...
app.get('/mf/sofe-manifest.json', handleGetManifest);
app.get('/mf/import-map.json', handleGetManifest);
app.patch('/mf/update-import-map', (req, res) => {
  ...
  if (req.body.scopes) {
    if (getConfig().manifestFormat !== 'importmap') {
      return res.status(400).send(`Invalid import map in request body -- scopes are only supported with manifest format "importmap"`);
    }
  ...
  if (req.body.imports) {
    ...
    if (importUrlsToValidate.length > 0) {
      validImportUrlPromises = importUrlsToValidate.map(url => verifyValidUrl(req, url));
    }
  }
  ...
  if (req.body.scopes) {
    ...
    if (scopeUrlsToValidate.length > 0) {
      validScopeUrlPromises = scopeUrlsToValidate.map(url => verifyValidUrl(req, url));
    }
  }
...
app.get('/mf/health', healthEndpoint);
app.patch('/mf/services', function (req, res) {
  req.body = JSON.parse(req.body);
  let service, url;
  let env = getEnv(req);
  ...
  if (!service || service.trim().length === 0) {
    return res.status(400).send(`Invalid service key - "${service}" is an invalid service key`);
  }
  ...
  let packageDirLevel = req.query.packageDirLevel && req.query.packageDirLevel !== '' ? Math.floor(req.query.packageDirLevel) : 1;

  if (req.query.packageDirLevel && isNaN(packageDirLevel)) {
    return res.status(400).send(`Query parameter packageDirLevel (${packageDirLevel}) should be of type number`);
  }
...
// Modified for Firebase Function
// let server;
// if (process.env.NODE_ENV !== "test") {
//   server = app.listen(
