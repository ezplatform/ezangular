...
async function createBlobService(target) {
  const connectionString = target.azureConnectionString || process.env.AZURE_STORAGE_CONNECTION_STRING;
  const account = target.azureAccount || process.env.AZURE_STORAGE_ACCOUNT;
  const accessKey = target.azureAccessKey || process.env.AZURE_STORAGE_ACCESS_KEY;
  if (connectionString) {
...
exports.readManifest = async function (target) {
  ...
  const response = await streamToString(downloadBlockBlobResponse.readableStreamBody);
  return response;
};
